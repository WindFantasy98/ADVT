...
from advt.defence import BitDepthReduction, Randomization
...
def main():
    ...
    # defend initializing
    # bit_defend = BitDepthReduction(cnn_encoder, device)
    rand_defend = Randomization(cnn_encoder, device)
    ...
    for X, lbl in all_data_loader:
        ...
        # filtered_X = bit_defend.bit_depth_reduction(advX)
        filtered_X = rand_defend.input_transform(advX)
        pred0 = rnn_decoder(cnn_encoder(filtered_X))
        singlevideo_save_to_path(filtered_X, PATH_ADV_FRAMES)
        _, ind0 = pred0.max(1)
