...
from advt.attack import SparseAdv
...
def main():
    ...
    # attack initializing
    sparseadv = SparseAdv([cnn_encoder, rnn_decoder], device)

    for X, lbl in all_data_loader:
        advX = sparseadv.attack(X, lbl) # get adv sample
    ...
    print('l2 distance of average perturbation:', sparseadv.total_distance / sparseadv.total_num)
    print('------------------------End------------------------')
